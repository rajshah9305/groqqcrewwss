...
try {
  ...
  // Determine SSL requirement and configuration for Vercel/Neon/Supabase
  const isVercel = process.env.VERCEL === "1";
  const isNeonOrSupabase = connectionString.includes('neon.tech') || connectionString.includes('supabase.co');
  const requiresSSL = isVercel || isNeonOrSupabase || connectionString.includes('sslmode=require');
  
  const sslConfig = requiresSSL
    ? { rejectUnauthorized: false } // Required for self-signed certs on some providers like Neon
    : undefined;
  ...
  try {
    ...
    pool.end();
    // In production, throw error; in development, fall back to in-memory
...
// Declare taskData outside try block so it’s accessible in catch block
let taskData: {
userId: number;
title: string;
description: string;
...
const updatedAtValue = updates.updatedAt ? toDate(updates.updatedAt) : new Date();
...
if (existingIndex >= 0) {
  ...
  inMemoryState.userPreferences[existingIndex] = {
    ...
    autoSaveResults:
      prefs.autoSaveResults ?? existing.autoSaveResults,
    updatedAt: new Date(),
  };
} else {
